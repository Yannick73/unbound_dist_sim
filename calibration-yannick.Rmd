...
data_generation_configuration <- filter(
  ...
  !((data_family == "gaussian" |
      ...
      data_family == "gumbel"   # gumbel does skew, but sigma controls both skew 
     # and scale. So just use that aspect
  )
   & shape == "skewed")
)

sigma_y_list <- list(
  "gaussian" = c(3, 12), # if equal x_y_coeff, 0 FPR in y ~ x + z1 + z2
  "gumbel" = c(1.2, 12),
  "symlognormal" = c(1, 2, 4) 
)
# Given our aux is usually log, compare to log reference
y_intercept_list <- c(2, 7, 10)
x_y_coef_list <- c(0.6, 8, 0.1)
