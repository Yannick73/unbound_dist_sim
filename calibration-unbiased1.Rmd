...
```{r}
data_config_unbiased1 <- expand.grid(
  z1_x_coef = 0.5,
  z1_y_coef = 2,
  z2_y_coef = 2,
  z3_x_coef = 1,
...
data_config_unbiased1 <- family_filter(data_config_unbiased1, skew_filter, "skewed")
thin_filter <- c("gumbel")#, "exgaussian") # exgauss does not do wide well at all
data_config_unbiased1 <- family_filter(data_config_unbiased1, thin_filter, "thin")
# data_generation_configuration <- family_filter(data_generation_configuration, "exgaussian", "wide")

z1z2_y_unbiased1_list <- list(
  "gaussian" = list(c(5, 5), c(5, 5)),
  "student" = list(c(-2, -2), c(3, 3)),
  "skew_normal" = list(c(-1, 1), c(-1, 1), c(3, 3)), #skewed FPR too low
  "generalized_normal" = list(c(-2, -2), c(2, 2)),
  "asym_laplace" = list(c(12, -6), c(-0.5, -0.5), c(1, 1)),
  "exgaussian" = list(c(0.1, 0.1), c(0.1, 0.1), c(-0.1, -0.1)),
  "gumbel" = list(NA, c(-4, -4), c(0.5, 0.5)),
  "logistic" = list(c(-3, -3), c(9, 9)),
  "symlognormal" = list(c(3, 3), c(3, 3), c(1, 1))
)

x_y_unbiased1_list <- list(
  "gaussian"     = c(2.5, 3.1),
  "student"      = c(1.15, 3.2),
  "skew_normal"  = c(0.7, 1.15, 1.7),
  "generalized_normal" = c(1, 2),
  "asym_laplace" = c(0.5, 1.2, 0.65),
  "exgaussian"   = c(0.5,  3,   0.55),
  "gumbel"       = c(NA,  1.6, 0.6),
  "logistic"     = c(0.38, 1.2),
...
sigma_y_list <- list(
  ...
  "exgaussian" = list(c(1, 0.5), c(6, 0.5), c(1, 3)),
...
for (i in seq_len(nrow(data_config_unbiased1))) {
  family <- data_config_unbiased1$data_family[[i]]
  shape_name <- data_config_unbiased1$shape[[i]]
  shape_index <- shape_index_lookup[[shape_name]]
  
  data_config_unbiased1$z1_y_coef[[i]] <- z1z2_y_unbiased1_list[[family]][[shape_index]][1]
  data_config_unbiased1$z2_y_coef[[i]] <- z1z2_y_unbiased1_list[[family]][[shape_index]][2]
  
  data_config_unbiased1$sigma_y[[i]] <- sigma_y_list[[family]][[shape_index]]
  data_config_unbiased1$y_intercept[[i]] <- y_intercept_list[[shape_index]]
  if(is.na(data_config_unbiased1$x_y_coef[[i]])) {
    data_config_unbiased1$x_y_coef[[i]] <- x_y_unbiased1_list[[family]][[shape_index]]
  }
}

data_config_unbiased1$id <- as.numeric(
  rownames(data_config_unbiased1)
)
```
...
```{r}
fit_config_unbiased1 <- expand.grid(
  fit_family = c(
    # "gaussian"#, # FPR passt
    # "student"#, # FPR passt
...
fit_config_unbiased1$prior = c() # should stay empty for this
```
...
result_df <- full_simulation(
  data_gen_confs = data_config_unbiased1,
  data_gen_fun = basedag_data,
  fit_confs = fit_config_unbiased1,
  metrics = metrics,
  ncores_simulation = 1,
