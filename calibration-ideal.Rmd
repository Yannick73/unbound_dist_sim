...
for (i in seq_len(nrow(data_config_ideal))) {
  family <- data_config_ideal$data_family[[i]]
  shape_name <- data_config_ideal$shape[[i]]
  shape_index <- shape_index_lookup[[shape_name]]
  
  data_config_ideal$z1_y_coef[[i]] <- z1z2_y_ideal_list[[family]][[shape_index]][1]
  data_config_ideal$z2_y_coef[[i]] <- z1z2_y_ideal_list[[family]][[shape_index]][2]
  
  data_config_ideal$sigma_y[[i]] <- sigma_y_list[[family]][[shape_index]]
  data_config_ideal$y_intercept[[i]] <- y_intercept_list[[shape_index]]
  if(is.na(data_config_ideal$x_y_coef[[i]])) {
    data_config_ideal$x_y_coef[[i]] <- x_y_ideal_list[[family]][[shape_index]]
  }
}

data_config_ideal$id <- as.numeric(
  rownames(data_config_ideal)
)
```
...
fit_config_ideal$prior = c() # should stay empty for this
```
...
result_df <- full_simulation(
  data_gen_confs = data_config_ideal,
  data_gen_fun = basedag_data,
  fit_confs = fit_config_ideal,
  metrics = metrics,
  ncores_simulation = 1,
